...
class PostDataHandler:
    def __init__(self, post_data_csv: str, replies_data_csv: str):
        ...
        logging.info("PostDataHandler initialized with post_data_csv: %s and replies_data_csv: %s", post_data_csv, replies_data_csv)
        ...
        for query in queries:
            ...
            for i in range(iterations_per_query):
                ...
                if not response or not response.get("data"):
                    logging.warning("No data found for query: %s on iteration %d", query, i+1)
                    continue
                ...
                for edges in response["data"]["searchResults"]["edges"]:
                    ...
                    for threads_item in edges["node"]["thread"]["thread_items"]:
                        ...
                        if post_id in seen_post_ids:
                            logging.debug("Post ID %s already processed, skipping", post_id)
                            continue
    ...
    def _extract_post_data(self, post_data, thread_type):
        try:
            ...
            text_fragments = post_data['text_post_app_info']['text_fragments'].get('fragments', [])
    ...
    def collect_replies_by_post_id(self, post_ids: list):
        ...
        for post_id in post_ids:
            ...
            try:
                response = json.loads(self.threads_interface.retrieve_replies_by_post_id(int(post_id)))
            except:
                response = ""
            if not response or not response.get("data"):
            ...
            for edges in response["data"]["data"]["edges"]:
                for threads_item in edges["node"]["thread_items"]:
                    ...
                    if reply_id in seen_reply_ids:
                        logging.debug("Reply ID %s already processed, skipping", reply_id)
                    ...
                    if reply_data is not None:
                        reply_data = self._update_post_data_with_json(post_data, reply_data)
                        reply_data['Parent Post ID'] = post_id

                        logging.info("Collected reply ID: %s for post ID: %s", reply_id, post_id)
    ...
    def _update_post_data_with_json(self, post_data, post_dict):
        ...
        if json_data:
            post_dict.update({
                'Following': json_data['user']['friendship_status'].get('following', False),
                'Followed By': json_data['user']['friendship_status'].get('followed_by', False),
                'Can Reply': json_data['text_post_app_info'].get('can_reply', False),
                'Reply Control': json_data['text_post_app_info'].get('reply_control', 'everyone'),
